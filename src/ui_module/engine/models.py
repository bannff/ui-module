...
class Action(BaseModel):
    ...
    payload: Dict[str, Any] = Field(
        default_factory=dict, description="Context data for the action"
    )
    target: Optional[str] = Field(
        None, description="Target path or view ID if negotiation"
    )
...
class Component(BaseModel):
    ...
    children: Optional[List[Union["Component", str]]] = Field(
        None, description="Child components"
    )
    actions: Optional[Dict[str, Action]] = Field(
        None, description="Map of event names to Actions"
    )
...
class View(BaseModel):
    ...
    data: Dict[str, Any] = Field(
        default_factory=dict, description="Initial data for the view"
    )
...
class SessionState(BaseModel):
    ...
    session_id: str
    data: Dict[str, Any] = Field(
        default_factory=dict, description="Arbitrary session data"
    )
